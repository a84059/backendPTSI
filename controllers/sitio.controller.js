...
//concelhobydistrito
exports.concelhobydistrito = function (req, res) {
    const query = connect.con.query("SELECT DISTINCT concelho FROM sitio WHERE distrito = ? like '%20';", [req.params.distrito], function (error, rows, fields) {
        console.log(query.sql);
        if (error) {
            console.log(error);
            res.status(jsonMessages.db.dbError.status).send(jsonMessages.db.dbError);
...
exports.freguesiabyconcelho = function (req, res) {
    const query = connect.con.query("SELECT DISTINCT freguesia1 FROM sitio WHERE concelho = ? like '%20';", [req.params.concelho], function (error, rows, fields) {
        console.log(query.sql);
        if (error) {
            console.log(error);
            res.status(jsonMessages.db.dbError.status).send(jsonMessages.db.dbError);
